...
const ListaProyectos = () => {
              ...
              tarea.id === tareaId
                ? {
                    ...
                    asignados: tarea.asignados
                      ? [...new Set([...tarea.asignados, user.id])]
                      : [user.id],
                  }
                : tarea
            ),
  ...
  if (!user) return <p>No estás autenticado. Inicia sesión para continuar.</p>;
        ...
        proyectosVisibles.map((proyecto) => (
          ...
          >
            ...
            <button onClick={() => toggleExpanded(proyecto.id)}>
              {expanded[proyecto.id] ? "Ocultar tareas" : "Mostrar tareas"}
            </button>
            {expanded[proyecto.id] && (
              ...
              >
                {proyecto.tareas.map((tarea) => (
                  <li key={tarea.id} style={{ margin: "0.4rem 0" }}>
                    {tarea.nombre} —{" "}
                    {tarea.asignados?.includes(user.id) ? (
                      <strong>Ya estás asignado</strong>
                    ) : (
                      <button
                        onClick={() => handleAsignar(proyecto.id, tarea.id)}
                      >
                        Asignarme
                      </button>
                    )}
