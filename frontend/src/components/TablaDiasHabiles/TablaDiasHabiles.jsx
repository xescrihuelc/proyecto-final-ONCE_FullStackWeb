...
const TablaDiasHabiles = () => {
  ...
  return (
    <div className="tabla-horas-container">
      <h3>Horas imputadas: {totalHoras.toFixed(1)} / {horasObjetivo} h</h3>
      <div className="progreso-container">
        <div className="progreso-barra" style={{ width: `${progreso}%` }} />
      </div>
      <table className="tabla-horas">
        <thead>
          <tr><th>Fecha</th><th>Día</th><th>Horas</th></tr>
        </thead>
        <tbody>
          {dias.map((d) => (
            <tr key={d.fecha} className={d.control === 0 ? "inhabil" : ""}>
              <td>{d.fecha}</td>
              <td>{d.dia}</td>
              <td>
                {d.control ? (
                  <input
                    ...
                    onChange={(e) => handleChange(d.fecha, e.target.value)}
                  />
                ) : (
                  <em>No laborable</em>
                )}
